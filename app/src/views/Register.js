        ...
        <>
            <section style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100vh', width: '100vw', flexDirection: 'column', background: 'linear-gradient(to left, #bdc3c7, #2c3e50)', border: 'none' }}>
                <div style={{ top: '0', left: '0', position: 'fixed', alignItems: 'center', justifyContent: 'space-between', display: 'flex', padding: '0 30px', height: '10vh', width: '100%' }}>
                    ...
                    <Link to="/login" style={{ textDecoration: 'none', color: '#fff', fontSize: 'large', fontWeight: '600', textShadow: '1px 1px 50px #000', borderBottom: '2px solid whitesmoke' }}>Login</Link>
                </div>
                <form onSubmit={submitForm} className='login__form'>
                    ...
                    {
                        isLoading ?
                            <div className="spinner-border spinner-border-sm" style={{ backgroundColor: 'transparent', color: 'whitesmoke', fontSize: 'medium', fontWeight: '500' }} role="status"></div>
                            : <button type='submit' className="login__form__submit__btn">Register</button>
                    }
                </form>
            </section>
        </>
